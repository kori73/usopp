...
def test_minmax_scaler_np(array):
    ...
    np.testing.assert_allclose(scaler.fit(array).transform(array), scaler.fit_transform(array), rtol=1e-06, atol=1e-06)
    np.testing.assert_allclose(array, scaler.inv_transform(scaler.transform(array)), rtol=1e-06, atol=1e-06)
...
def test_minmax_scaler_series(series):
    ...
    np.testing.assert_allclose(series, scaler.inv_transform(scaler.transform(series)), rtol=1e-06, atol=1e-06)
...
def test_minmax_scaler_df(df):
    ...
    np.testing.assert_allclose(scaler.fit(df).transform(df), scaler.fit_transform(df), rtol=1e-06, atol=1e-06)
    np.testing.assert_allclose(df, scaler.inv_transform(scaler.transform(df)), rtol=1e-06, atol=1e-06)
